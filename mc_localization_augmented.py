...
class MonteCarloLocalization:
    ...
    def __init__(self, num_particles=300, alpha1=0.05, alpha2=0.05, alpha3=0.01, alpha4=0.001, sigma_hit=0.1,
    ...
    def sample_motion_model(self, u, x_prev):
        ...
        delta_trans_hat = delta_trans - self.sample_normal(
            self.alpha3 * delta_trans + self.alpha4 * (np.abs(delta_rot1) + np.abs(delta_rot2)))
    ...
    def resample(self, w):
        ...
        for indx in random_indx:
            ...
            while not self.occupancy.is_free((new_x, new_y)) or self.occupancy.is_unknown((new_x, new_y)):
                # print("Not free ", x[i], y[i])
                new_x = np.random.uniform(0, self.map_width*self.map_resolution-self.map_resolution)
                new_y = np.random.uniform(0, self.map_height*self.map_resolution-self.map_resolution)
            self.particles[0, indx] = new_x
